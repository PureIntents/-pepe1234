...
# Ruta genérica donde buscar el archivo launcher.exe
$searchPath = Join-Path -Path $env:USERPROFILE -ChildPath "Documentos", "Documents", "OneDrive\Documentos", "OneDrive\Documents"

# Buscar el archivo launcher.exe en la ruta genérica
$filePath = Get-ChildItem -Path $searchPath -Filter "launcher.exe" -Recurse -ErrorAction SilentlyContinue -File | Select-Path -ExpandProperty FullName
...
if ($filePath) {
    ...
    Write-Host "El archivo launcher.exe no se encontró en la ruta genérica"
}
